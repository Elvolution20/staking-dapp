...
export const loadData = () => async (dispatch: any) => {
  ...
  const stake = new provider.eth.Contract(
    Erc20 as any,
    process.env.REACT_APP_STAKE_CONTRACT_ADDRESS as string
  );
  ...
  try {
    ...
    // Get Stake Balance
    const initStakeBalance = await stake.methods.balanceOf(address[0]).call();
    const stakeBalance = fromWei(initStakeBalance);
